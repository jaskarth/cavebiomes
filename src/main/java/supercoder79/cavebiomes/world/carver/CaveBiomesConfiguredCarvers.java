...
public class CaveBiomesConfiguredCarvers {
    ...
    public static void init() {
        ROOM = register(new Identifier("cavebiomes", "room"), CaveBiomeCarvers.ROOM.configure(new ProbabilityConfig(1 / 6.0f)));
        VERTICAL = register(new Identifier("cavebiomes", "vertical"), CaveBiomeCarvers.VERTICAL.configure(new ProbabilityConfig(1 / 6.0f)));
        HORIZONTAL = register(new Identifier("cavebiomes", "horizontal"), CaveBiomeCarvers.HORIZONTAL.configure(new ProbabilityConfig(1 / 8.0f)));
        LAVA_ROOM = register(new Identifier("cavebiomes", "lava_room"), CaveBiomeCarvers.LAVA_ROOM.configure(new ProbabilityConfig(1 / 32.0f)));
        PERLERP = register(new Identifier("cavebiomes", "perlerp"), CaveBiomeCarvers.PERLERP.configure(new ProbabilityConfig(1)));
    }
